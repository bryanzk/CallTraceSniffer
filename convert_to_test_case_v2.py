#!/usr/bin/env python3
"""
将 invocation flow 数据转换为 test_cases.yaml 格式 (改进版)
从原始数据中提取更准确的 Swaps、Transfers 和 ExecutionTree 信息
"""
import json
...
def extract_transfers_from_data(data_map: Dict) -> List[Dict]:
    """从dataMap中提取所有transfer调用"""
    transfers = []
    # transfer的selector: 0xa9059cbb
    ...
    for node_id, node_data in data_map.items():
        ...
        # 检查是否为transfer调用
        is_transfer = False
        if selector and selector.lower() == TRANSFER_SELECTOR.lower():
            is_transfer = True
...
def extract_swaps_from_data(data_map: Dict, main_trace: List) -> List[Dict]:
...
def build_execution_tree_simplified(swaps: List, main_trace: List, data_map: Dict) -> Dict:
...
def generate_test_case_format(tx_hash: str, swaps: List, execution_tree: Dict, transfers: List) -> str:
...
def main():
    ...
    try:
        ...
        print("错误: 未找到 invocation_flow_data.json 文件")
        return
    
    trace_data = raw_data.get('trace_data')
    if not trace_data:
    ...
    # 构建简化的ExecutionTree（只显示Swaps相关节点）
    ...
    print(f"\n✓ 已生成 test_case 格式文件: {output_file}")
    print("\n" + "=" * 80)
    print("输出预览:")
    print("=" * 80)
    print(output)
